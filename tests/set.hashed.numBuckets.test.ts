...
describe('BigBoiSetHashed numBuckets functionality', () => {
  it('should correctly determine the number of buckets based off of the max number of keys', async () => {
    const set1 = new BigBoiSetHashed({ maxKeys: 2 ** 25 });
    expect(set1.numBuckets).toBe(2);
    
    const set2 = new BigBoiSetHashed({ maxKeys: MAX_KEY_COUNT * 2 });
    expect(set2.numBuckets).toBe(2);

    const set3 = new BigBoiSetHashed({ maxKeys: (MAX_KEY_COUNT * 2) + 1 });
    expect(set3.numBuckets).toBe(3);

    const set4 = new BigBoiSetHashed({ maxKeys: (MAX_KEY_COUNT * 3) + 1 });
  ...
  it('should give a console warning if the max number of keys is less than the max number of unique keys allowed in a Set', async () => {
    ...
    const set = new BigBoiSetHashed({ maxKeys: MAX_KEY_COUNT });
  ...
  it('should error if the max number of keys is greater than Number.MAX_SAFE_INTEGER', async () => {
    expect(() => new BigBoiSetHashed({ maxKeys: Number.MAX_SAFE_INTEGER + 1 })).toThrow('maxKeys must be less than Number.MAX_SAFE_INTEGER.');
  });
  ...
  // to create 536,870,912 buckets. So extrapolating, we'd need ~80GB of memory to create a BigBoiSetHashed with MAX_SAFE_INTEGER
  // keys.
  // it('should be okay if the max number of keys is equal to Number.MAX_SAFE_INTEGER', async () => {
  //   expect(() => new BigBoiSetHashed({ maxKeys: Number.MAX_SAFE_INTEGER })).not.toThrow();
  // });
});
